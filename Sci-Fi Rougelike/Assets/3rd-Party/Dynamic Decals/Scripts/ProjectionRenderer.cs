...
namespace LlockhamIndustries.Decals
{
    ...
    public class ProjectionRenderer : MonoBehaviour
    {
        ...
        */
        ...
        //Sub-Order
        public ProjectionData Data { get; set; }
        ...
        */
        ...
        {
            marked = true;
            if (UpdateImmediately && Renderer != null) UpdateProperties();
        }
        ...
        */
        ...
        public void UpdateProjection()
        {
            if (Renderer != null)
                if (Projection != null && Projection.Valid)
                {
                    ...
                    //Apply materials
                    Renderer.sharedMaterials = Projection.Materials;
        ...
        private void UpdateRendererBlock(ProjectionProperty[] Local, ProjectionProperty[] Global)
        {
            ...
            for (var i = 0; i < Local.Length; i++)
            {
                ...
                {
                    var value = Local[i].enabled ? Local[i].value : Global[i].value;
                    block.SetFloat(Global[i].nameID, value);
                }
                if (Local[i].type == PropertyType.Color)
                {
                    var color = Local[i].enabled ? Local[i].color : Global[i].color;
                    block.SetColor(Global[i].nameID, color);
                }
                if (Local[i].type == PropertyType.Combo)
                {
                    var color = Local[i].enabled ? Local[i].color * Local[i].value : Global[i].color * Global[i].value;
                    block.SetColor(Global[i].nameID, color);
                }
            }
        }
        ...
        private void Initialize()
        {
            if (projection != null)
            {
                ...
                //Register & Initialize renderer
                if (Register()) InitializeRenderer();
            }
