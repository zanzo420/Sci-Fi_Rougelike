...
namespace LlockhamIndustries.Decals
{
    ...
    */
    ...
    public class DynamicDecals : MonoBehaviour
    {
        ...
        public static DynamicDecals System
        {
            get
            {
                ...
                {
                    //Create our system
                    var go = new GameObject("Dynamic Decals");
        ...
        private void PreShaderReplacement(Camera Camera, CameraData Data)
        {
            if (Projections.Count > 0 && shaderReplacement)
            {
                ...
                var customCamera = Data.CustomCamera;
                ...
                var renderMaskSeperate = Data.replacement != ShaderReplacement.Standard || SystemInfo.supportedRenderTargetCount < 3;
            ...
            switch (Data.replacement)
            {
                ...
                case ShaderReplacement.Standard:
                    if (RenderMaskSeperate)
                    {
                        ...
                        RenderBuffer[] buffers = { Data.Depth.colorBuffer, Data.Normals.colorBuffer };
                    ...
                    else
                    {
                        ...
                        RenderBuffer[] buffers = { Data.Depth.colorBuffer, Data.Normals.colorBuffer, Data.MaskBuffers[0].colorBuffer };
                        CustomCamera.SetTargetBuffers(buffers, Data.Depth.depthBuffer);
        ...
        {
            var width = VRSettings.enabled && OriginalCamera.stereoEnabled && Settings.SinglePassVR ? VRSettings.eyeTextureWidth * 2: CustomCamera.pixelWidth;
            ...
            //Mask
            for (var i = 0; i < Data.MaskBuffers.Length; i++)
            {
                var depthBuffer = i == 0 && RenderMaskSeperate ? 24 : 0;
        ...
        private void UpdateSystemPath()
        {
            ...
            if (target != null)
                ...
                {
                    var newPath = SystemPath.Forward;
        ...
        public void RestoreDepthTextureModes()
        {
            ...
            {
                var camera = cameraData.ElementAt(i).Key;
        ...
        public bool Register(ProjectionRenderer Instance)
        {
            if (Instance != null)
            {
                ...
                var data = GetProjectionData(projection);
                if (data != null)
                {
                    data.Add(Instance);
                    return true;
        ...
        public int RendererCount
        {
            ...
            {
                var count = 0;
                for (var i = 0; i < Projections.Count; i++) count += Projections[i].instances.Count;
                return count;
            }
        }
...
#if UNITY_EDITOR
        private readonly List<GameObject> dragables = new List<GameObject>();
        ...
        private void OnSceneGUI(SceneView sceneView)
        {
                ...
                if (dragables.Count == 0)
                    foreach (var o in DragAndDrop.objectReferences)
                    {
                        ...
                        if (go != null)
                            ...
                            {
                                //Create our dragable
                                var dragable = PrefabUtility.InstantiatePrefab(go) as GameObject;
            ...
            if (Event.current.type == EventType.DragExited && dragables.Count > 0)
            {
                ...
                foreach (var go in dragables) DestroyImmediate(go);
                dragables.Clear();
    ...
    internal class CameraData
    {
        ...
        //Depth texture mode
        public DepthTextureMode? originalDTM, desiredDTM;
        ...
        public Camera CustomCamera
        {
            get
            {
                ...
                {
                    var cameraObject = new GameObject("Custom Camera");
                    ...
                    if (Application.isPlaying) Object.DontDestroyOnLoad(cameraObject);
                }
                return customCamera;
            }
        }
